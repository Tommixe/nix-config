...
# Attention! private key must be saved in bw as a single line using "cat ~/.ssh/id_rsa | tr -d '\n' 
...
nix run github:numtide/nixos-anywhere -- --build-on-remote --option nixos-rebuild fast --extra-files "$temp" --flake ..\#$host root@$toip --debug
